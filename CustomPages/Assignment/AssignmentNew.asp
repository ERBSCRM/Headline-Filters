...
record=CRM.CreateRecord("Assignment");
if( false )
  record.SetWorkFlowInfo("Assignment Workflow", "Logged");

EntryGroup=CRM.GetBlock("AssignmentNewEntry");
EntryGroup.Title="Assignment";
...
if( context == iKey_CompanyId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_CompanyId");
...
else if( context == iKey_PersonId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_PersonId");
...
else if( context == iKey_UserId && true )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_UserId");
...
else if( context == iKey_ChannelId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_ChannelId");
...
else if( context == iKey_LeadId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_LeadId");
...
else if( context == iKey_OpportunityId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_OpportunityId");
...
else if( context == iKey_OrderId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_OrderId");
...
else if( context == iKey_QuoteId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_QuoteId");
...
else if( context == iKey_CaseId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_CaseId");
...
else if( context == iKey_AccountId && false )
{
  ...
  {
    Entry = EntryGroup.GetEntry("assi_AccountId");
...
if(CRM.Mode==Save)
  Response.Redirect("AssignmentSummary.asp?J=Assignment/AssignmentSummary.asp&E=Assignment&assi_AssignmentID="+record("assi_AssignmentID")+"&"+Request.QueryString);
else
{
